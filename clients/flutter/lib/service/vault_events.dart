...
class VaultEvents {
	...
	void subscribe(void Function(String workId) listener) => _listeners.add(listener);

	void unsubscribe(void Function(String workId) listener) => _listeners.remove(listener);
			...
			while (isActive && generation == _generation) {
				try {
					...
					if (response.statusCode != 200) {
						throw HttpException('event stream refused (${response.statusCode})');
					}
