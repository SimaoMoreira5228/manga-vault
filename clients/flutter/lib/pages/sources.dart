...
class _SourcesPageState extends State<SourcesPage> {
	...
	Future<void> _load() async {
		...
		final loadedCatalog = await widget.vault.pluginCatalog().catchError((_) => <CatalogItem>[]);
		if (!mounted) return;
		setState(() {
			repos = loadedRepos;
		...
		final url = await showDialog<String>(
			...
			builder: (context) => AlertDialog(
				...
				content: TextField(
					...
					autofocus: true,
					decoration: const InputDecoration(hintText: 'https://example.org/repo.json'),
				),
				actions: [
				...
				slivers: [
					const SliverToBoxAdapter(
						child: Padding(
							...
							child: Text('REPOSITORIES', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, letterSpacing: 0.6)),
						),
					),
					_buildRepos(),
					const SliverToBoxAdapter(
						child: Padding(
							padding: EdgeInsets.fromLTRB(16, 16, 16, 8),
							child: Text('CATALOG', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, letterSpacing: 0.6)),
						),
					),
					_buildCatalog(),
				],
			...
			return SliverList(
				delegate: SliverChildListDelegate([
					Padding(
						...
						child: Text('No repositories configured. Add one to browse its plugins.'),
					),
				]),
			);
		}
		return SliverList(
			delegate: SliverChildBuilderDelegate(
				(context, index) => ListTile(
					...
					trailing: IconButton(icon: const Icon(Icons.delete_outline), onPressed: () => _removeRepo(shown[index].id)),
				),
	...
	SliverList _buildCatalog() {
		final shown = catalog;
		if (shown == null) return SliverList(delegate: SliverChildListDelegate([Center(child: CircularProgressIndicator())]));
		if (shown.isEmpty) {
			return SliverList(
				delegate: SliverChildListDelegate([
					Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('Nothing in the catalog yet.')),
				]),
			);
		}
		return SliverList(
...
class CatalogTile extends StatelessWidget {
	...
	Widget build(BuildContext context) {
		...
		return ListTile(
			title: Text(item.id),
			subtitle: Text('${item.repoName} · v${item.availableVersion} · ${item.backend}'),
			trailing: FilledButton.tonal(
				style: FilledButton.styleFrom(
					foregroundColor: installed && !item.updateAvailable ? scheme.error : scheme.primary,
				),
