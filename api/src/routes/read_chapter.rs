...
#[derive(serde::Deserialize)]
struct MarkAllAsread {
	pub user_id: i32,
	pub manga_id: i32,
}

#[derive(serde::Deserialize)]
struct MarkAllAsUnread {
	pub user_id: i32,
	pub manga_id: i32,
}
...
#[post("user/{user_id}/read/{manga_id}/mark-all-as-read")]
async fn mark_all_as_read(db: web::Data<connection::Connection>, params: web::Json<MarkAllAsread>) -> impl Responder {
	let user: Option<crate::entities::users::Model> = crate::entities::users::Entity::find_by_id(params.user_id)
		.one(db.get_ref())
		.await
		.unwrap();
	...
	HttpResponse::Ok().body("All chapters marked as read")
}
...
#[post("user/{user_id}/read/{manga_id}/mark-all-as-unread")]
async fn mark_all_as_unread(db: web::Data<connection::Connection>, params: web::Json<MarkAllAsUnread>) -> impl Responder {
	let user: Option<crate::entities::users::Model> = crate::entities::users::Entity::find_by_id(params.user_id)
		.one(db.get_ref())
		.await
		.unwrap();
	...
	HttpResponse::Ok().body("All chapters marked as unread")
}
...
pub fn init_routes(cfg: &mut web::ServiceConfig) {
	...
	cfg.service(mark_all_as_read);
	cfg.service(mark_as_unread);
	cfg.service(mark_all_as_unread);
	cfg.service(is_read);
