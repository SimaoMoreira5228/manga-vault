...
>
	{#snippet content()}
		...
		<article>
			<input type="text" bind:value={isUpdatingCategory.input.username} class="input" />
		</article>
		<footer class="flex justify-end gap-4">
			<button
				type="button"
				class="btn preset-tonal"
				onclick={() => (isUpdatingCategory.open = false)}>Cancel</button
			>
			<button type="button" class="btn preset-filled" onclick={updateProfile}>Confirm</button>
		</footer>
	{/snippet}
</Modal>
