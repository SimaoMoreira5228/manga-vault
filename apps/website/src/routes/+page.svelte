...
import { type FavoriteMangaShell, load } from "$lib/utils/personalLibrary";
...
async function getfavoriteMangas(categoryId: number) {
	...
	try {
		...
		toaster.error({ title: "Error", description: "Failed to fetch favorite mangas" });
	} finally {
						...
						{#each categories as category (category.id)}
							<Tabs.Panel value={category.id.toString()} base="h-full w-full">
								...
								{:else}
									...
									>
										{#each favoriteMangas as favoriteManga (favoriteManga.id)}
											<a
                          ...
                          favoriteManga.manga.url
                            .split('/')
                            .slice(0, 3)
                            .join('/') + '/'
                        )});"
												href={resolve(`/manga/${favoriteManga.manga.id}`)}
											>
												...
												{#if favoriteManga.manga.chaptersAmount - favoriteManga.manga.userReadChaptersAmount > 0}
													<span class="badge-icon preset-filled-primary-500 absolute -right-0 -top-0 z-10">
														{
	...
	{#snippet content()}
		<header class="flex justify-between">
			<h4 class="h4">
				Edit Category ({
					categories.find(
						(c) => c.id.toString() === isEditingCategory.id,
					)?.name
				})
			</h4>
		</header>
		...
		<footer class="flex justify-end gap-4">
			...
			<button
				...
				onclick={() => (isEditingCategory.open = false)}
			>
				Cancel
			</button>
			<button type="button" class="btn preset-filled" onclick={updateCategory}>
				Confirm
			</button>
		</footer>
	{/snippet}
	...
	{#snippet content()}
		...
		<footer class="flex justify-end gap-4">
			<button
				type="button"
				class="btn preset-tonal"
				onclick={() => (isCreatingCategory.open = false)}
			>
				Cancel
			</button>
			<button type="button" class="btn preset-filled" onclick={createCategory}>
				Confirm
			</button>
		</footer>
	{/snippet}
