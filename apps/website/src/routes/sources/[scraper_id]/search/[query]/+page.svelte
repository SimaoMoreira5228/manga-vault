...
async function loadSearch() {
	...
	try {
		...
		if (result.error) {
			console.error(`Failed to Search for "${searchQuery}": ${result.error.message}`);
			toaster.error({ title: "Error", description: `Failed to Search for "${searchQuery}"` });
			lastLoadFailed = true;
			return 0;
		}
		...
		console.error("Unexpected error in loadSearch:", error);
		lastLoadFailed = true;
		...
		>
			{#each items as item, i (i)}
				{#if item.mangaId}
					<MangaCard
						work={item}
						href={resolve(`/${scraper?.type?.toLowerCase() === "novel" ? "novel" : "manga"}/${item.mangaId}`)}
