...
async function handleSubmit(event: Event) {
	...
	const result = input.safeParse({ username: rawUsername, password: rawPassword });
	...
	try {
		...
		if (err && typeof err === "object" && "message" in err && typeof (err as any).message === "string") {
			formError = (err as { message: string }).message;
		} else {
	...
	>
		<header class="text-center">
			...
			<p class="mt-1 text-sm opacity-70">
				...
				<a href={resolve("/register")} class="anchor">Create one</a>.
			</p>
		</header>
