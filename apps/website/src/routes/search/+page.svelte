<script lang="ts">
	...
	let scrapersState = $state<
		Record<string, { open: boolean; items: MangaShell[]; loading: boolean; lastQuery: string }>
	>({});
...
<div class="flex h-full w-full items-center justify-center p-8">
	...
	{:else}
		<div class="flex h-full w-full flex-col items-center justify-center gap-4">
			...
			>
				<input class="input min-w-8/10" type="text" placeholder="Search" bind:value={searchQuery} />
							...
							{#snippet panel()}
								...
								{:else}
									<div class="flex w-full flex-row flex-nowrap items-start gap-2 overflow-x-auto">
										{#each scrapersState[scraper.id]?.items as item}
											<a
