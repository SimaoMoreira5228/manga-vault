...
pub async fn export_backup(State(state): State<AppState>, auth: Authenticated) -> Result<axum::response::Response, ApiError> {
	...
	let response = axum::response::Response::builder()
		...
		.header(header::CONTENT_DISPOSITION, "attachment; filename=\"manga-vault-backup.json\"")
		.body(axum::body::Body::from(body))
		.map_err(|e| ApiError::bad_request(e.to_string()))?;
	Ok(response)
}

pub async fn import_backup(State(state): State<AppState>, auth: Authenticated, Json(data): Json<application::backup::BackupData>) -> Result<Json<serde_json::Value>, ApiError> {
	require_secret(&state)?;
