...
use crate::{Chapter, Manga, MangaItem, ScrapperTraits, ScrapperType};
...
impl ScrapperTraits for MangaReadOrgScrapper {
    ...
    async fn scrape_latest(&self, page: u16) -> Result<Vec<MangaItem>, reqwest::Error> {
        ...
        for mangas_div in html.select(&mangas_div_selector) {
            let content_divs_selector =
                scraper::Selector::parse("div.c-tabs-item__content").unwrap();
        ...
        for mangas_div in html.select(&mangas_div_selector) {
            ...
            for div in contet_divs {
                let img_url_div = div
                    ...
                    .next()
                    .unwrap();
                let img_url = self::MangaReadOrgScrapper::get_image_url(&img_url_div);
    ...
    async fn scrape_manga(&self, url: &str) -> Result<Vec<Manga>, reqwest::Error> {
        ...
        let img_url =
            self::MangaReadOrgScrapper::get_image_url(&html.select(&img_selector).next().unwrap());
