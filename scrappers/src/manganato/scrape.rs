...
impl ScrapperTraits for ManganatoScrapper {
	async fn get_cookies(&self) -> Result<String, reqwest::Error> {
		...
		let response = client.get("https://manganato.com/").send().await?;
	...
	async fn scrape_chapter(&self, url: &str) -> Result<Vec<String>, reqwest::Error> {
		let cookies = ManganatoScrapper::get_cookies(self).await.unwrap();
		...
		let img_div_selector = scraper::Selector::parse("div.container-chapter-reader img").unwrap();
		let mut imgs: Vec<String> = Vec::new();

		for img in html.select(&img_div_selector) {
			let img_url = img.value().attr("src");
			if img_url.is_none() {
				continue;
			}
			let img_url = img_url.unwrap();
			imgs.push(img_url.trim().to_string());
		}
	...
	async fn scrape_manga(&self, url: &str) -> Result<MangaPage, reqwest::Error> {
		...
		for line in lines_clone {
			if line.contains("Alternative") {
				...
				alternative = next_line.split(" ; ").map(|x| x.trim().to_string()).collect();
			} else if line.contains("Author(s)") {
				let next_line = lines[lines.iter().position(|x| *x == line).unwrap() + 1];
				authors = next_line.split(" - ").map(|x| x.trim().to_string()).collect();
			} else if line.contains("Status") {
				let next_line = lines[lines.iter().position(|x| *x == line).unwrap() + 1];
				status = next_line.trim().to_string();
			} else if line.contains("Genres") {
				let next_line = lines[lines.iter().position(|x| *x == line).unwrap() + 1];
				genres = next_line.split(" - ").map(|x| x.trim().to_string()).collect();
			}
		}
		...
		description = description.split_once(':').unwrap().1.trim().to_string();
	...
	async fn get_info(&self) -> Result<crate::ScrapperInfo, reqwest::Error> {
		Ok(crate::ScrapperInfo {
			id: ScrapperType::Manganato,
			name: "Manganato".to_string(),
			img_url: "https://manganato.com/favicon.ico".to_string(),
		})
	}
