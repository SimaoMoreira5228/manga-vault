...
fn get_image_url(img_element: &scraper::ElementRef) -> String {
    ...
    if attrs.contains_key("data-src") {
        return attrs.get("data-src").unwrap_or(&"").trim().to_string();
    } else if attrs.contains_key("src") {
        return attrs.get("src").unwrap_or(&"").trim().to_string();
    } else if attrs.contains_key("data-cfsrc") {
        return attrs.get("data-cfsrc").unwrap_or(&"").trim().to_string();
    } else if attrs.contains_key("data-lazy-src") {
        return attrs.get("data-lazy-src").unwrap_or(&"").trim().to_string();
    } else {
        return "".to_string();
    }
}
